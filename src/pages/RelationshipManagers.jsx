...
const RelationshipManagers = () => {
  ...
  return (
    <div className="space-y-4 md:space-y-6 w-full max-w-full overflow-x-hidden px-4 md:px-0">
      {/* Header - single column on mobile, full-width stacked buttons */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-[20px] md:text-2xl font-bold text-gray-900 leading-tight">Relationship Managers</h1>
      ...
      {/* Stats - Mobile: 2 responsive cards (Total, Active) */}
      <div className="grid grid-cols-2 gap-3 md:hidden">
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 text-center">
          <p className="text-xs text-gray-500 font-medium">Total</p>
          <p className="text-lg font-bold text-gray-900 mt-1">{totalRMs}</p>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 text-center">
          <p className="text-xs text-gray-500 font-medium">Active</p>
          <p className="text-lg font-bold text-green-600 mt-1">{activeRMs}</p>
        </div>
      </div>
      ...
      {/* Filters - Collapsible accordion, full-width inputs, touch-friendly */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="sm:col-span-2 lg:col-span-1">
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="Name, location, owner, email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2.5 md:py-2 min-h-[44px] md:min-h-0 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Status</label>
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full px-3 py-2.5 md:py-2 min-h-[44px] md:min-h-0 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm bg-white">
                  {statusOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">City</label>
                <select value={cityFilter} onChange={(e) => setCityFilter(e.target.value)} className="w-full px-3 py-2.5 md:py-2 min-h-[44px] md:min-h-0 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm bg-white">
                  {cityOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">State</label>
                <select value={stateFilter} onChange={(e) => setStateFilter(e.target.value)} className="w-full px-3 py-2.5 md:py-2 min-h-[44px] md:min-h-0 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm bg-white">
      ...
      <div className="md:hidden space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-8 text-center">
            <p className="text-sm text-gray-500">No relationship managers found</p>
          </div>
        ) : (
          sortedRMs.map((rm) => {
            const id = rm.id || rm._id
              ...
              >
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-base font-bold text-gray-900 mt-0.5">{rm.name || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 font-medium">Location</p>
                    <p className="text-sm font-semibold text-gray-900 mt-0.5">{rm.address?.city || rm.address?.state || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 font-medium">Owner</p>
                    <p className="text-sm font-semibold text-gray-900 mt-0.5">{rm.ownerName || 'N/A'}</p>
                  </div>
        ...
        {!loading && sortedRMs.length > 0 && (
          <p className="text-sm text-gray-600 text-center py-2">
            Showing <span className="font-medium">{sortedRMs.length}</span> of <span className="font-medium">{sortedRMs.length}</span> relationship managers
          </p>
        )}
      </div>
